...
test('[Nano S] Mint agEUR with USDC with sender != user and ledger flag set', zemu("nanos", async (sim, eth) => {
  const contract = new ethers.Contract(contractAddr, abi);
...
test('[Nano S] Mint agEUR with USDC with sender == user', zemu("nanos", async (sim, eth) => {
  ...
  await sim.navigateAndCompareSnapshots('.', 'nanos_mint_ageur_usdc_with_equal_sender_and_user', [9, 0]);
